...
from app.models import WebCalidadTEC, EvaluacionesTEC, Nomina, EvaluacionesADM, WebCalidadADM
...
def load_inde():
    ...
    # for u in WebCalidadTEC.query.all():
    #     # print(u.__dict__)
    #     # print(WebCalidadTEC.row2dict(u))
    #     d.append(u.__dict__)
    # time_end = time.time()
    ...
    for row in WebCalidadTEC.query.filter_by(TipoEvaluacion=perfil_calidad()):
        row_dict = row.__dict__
        row_dict.pop('_sa_instance_state', None)
        row_dict['Calificacion'] = calcula_nota(row_dict, 'TEC')
        db.session.add(EvaluacionesTEC(**row_dict))

    EvaluacionesADM.query.filter_by(TipoEvaluacion=perfil_calidad()).delete()
    for row in WebCalidadADM.query.filter_by(TipoEvaluacion=perfil_calidad()):
        row_dict = row.__dict__
        row_dict.pop('_sa_instance_state', None)
        row_dict['Calificacion'] = calcula_nota(row_dict, 'ADM')
        db.session.add(EvaluacionesADM(**row_dict))
...
def adm():
    lista = []
    evaluaciones = db.session.query(EvaluacionesADM.Asesor,
                                    func.round(func.avg(EvaluacionesADM.Calificacion), 2).label('Promedio_global')) \
        .group_by(EvaluacionesADM.Asesor).order_by(desc('Promedio_global')).all()

    evaluaciones_cant = len(evaluaciones)

    for i, row in enumerate(evaluaciones):
        # lista[row.Asesor] = dict(zip(row.keys(), row))  # Tuple result to dict
        nota_interno = tuple_to_value(db.session.query(func.round(func.avg(EvaluacionesADM.Calificacion), 2).label('Interno')
                                                       ).filter_by(Asesor=row.Asesor,
                                                                   TipoEvaluacion=perfil_calidad()).first())
        nota_cruzado = tuple_to_value(db.session.query(func.round(func.avg(EvaluacionesADM.Calificacion), 2).label('Cruzado')
                                                       ).filter_by(Asesor=row.Asesor,
                                                                   TipoEvaluacion=perfil_cruzado_mdy()).first())
        lista.append({
            'asesor': row.Asesor,
